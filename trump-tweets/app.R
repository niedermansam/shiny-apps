...
server <- function(input, output, session) {
  ...
  tweets <- jsonlite::fromJSON('trump-tweets.json') %>% as_tibble()

  #tweets$created_at <- tweets$created_at %>% str_remove_all("^... ") %>% mdy_hms()
  tweets$date <- tweets$created_at %>%
    str_remove_all("^... ") %>%
    str_remove_all("\\d{2}:\\d{2}:\\d{2} \\+\\d{4} ") %>%
    mdy()

  tweets$time <- tweets$created_at %>%
    str_extract("\\d{2}:\\d{2}:\\d{2}")

  tweets$created_at <- paste(tweets$date, tweets$time) %>% ymd_hms()
  ...
  days <- tibble(date = seq.Date(to = as.Date(tweets$date[1]), from = as.Date(tweets$date[nrow(tweets)]), by = "1 day"))
...
search.tweets <- reactive({
  ...
  tweets %>% filter(str_detect(text, phrase) & date >= start_date & date <= end_date & is_retweet <= retweets)
